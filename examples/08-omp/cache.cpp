...
void read_by_row_omp(int** array, size_t rows, size_t cols) {
    volatile int sum = 0;
...
void read_by_col_omp(int** array, size_t rows, size_t cols) {
    volatile int sum = 0;
#pragma omp parallel for reduction(+ : sum)
    for (size_t i = 0; i < cols; ++i) {
        for (size_t j = 0; j < rows; ++j) {
...
int main() {
    ...
    BENCHMARK(read_by_row_omp(array, SIZE, SIZE), "Read by row OMP: ");
    BENCHMARK(read_by_col_omp(array, SIZE, SIZE), "Read by col OMP: ");
